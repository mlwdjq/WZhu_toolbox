...
Tph = 2*pi/lambda_nm*(n1*thickness_nm*tan(th0) - n2*thickness_nm*tan(th1)*sin(th2));
...
[rs,ts] = fileInterference(rs01,ts01,rs12,ts12,Rph,Tph);
[rp,tp] = fileInterference(rp01,tp01,rp12,tp12,Rph,Tph);
Rs = abs(rs)^2;
Rp = abs(rp)^2;
Ts = abs(ts)^2;
Tp = abs(tp)^2;
...
function [r,t]= fileInterference(r01,t01,r12,t12,Rph,Tph)
r = r01 + t01*(1-r01)*r12*exp(-1i*2*Rph)/(1+r01*r12*exp(-1i*2*Rph));
